#!/usr/bin/env python3
"""
Harness Evolution entry point for configs under `configs/experiments/`.

For Harness Scaling and other settings, use `evolve.py` instead.
"""
...
DATASET_PATH_ALIASES = {
    # This Harbor install does not currently expose Terminal-Bench 2.1 as a
    # built-in dataset, so keep the config alias but run the checked-out pack.
...
def send_feishu_notification(config: dict, title: str, content: str) -> None:
    ...
    body = {
        ...
        "card": {
            "header": {
                "title": {"tag": "plain_text", "content": f"[Harness Evolution] {title}"},
                "template": "blue",
            },
            "elements": [
                {
...
def resolve_dataset_path_alias(dataset: str | None) -> Path | None:
    """Map AHE-friendly dataset aliases to local Harbor dataset pack paths."""
    alias_path = DATASET_PATH_ALIASES.get(dataset or "")
    if alias_path is None:
        return None
    resolved = Path(alias_path)
    if not resolved.is_absolute():
        resolved = (PROJECT_DIR / resolved).resolve()
    return resolved


def hide_test_results_from_agents(config: dict) -> bool:
    """Whether detailed verifier/test output should be withheld from LLM agents."""
    visibility = config.get("visibility", {})
    if isinstance(visibility, dict) and visibility.get("hide_test_results_from_agents"):
        return True
    return bool(config.get("hide_test_results_from_agents"))


HIDDEN_AGENT_METADATA_PATHS = (
...
@contextmanager
def hide_result_metadata_from_agents(exp_dir: Path, enabled: bool):
    """Temporarily move reward-derived experiment metadata out of agent reach."""
    if not enabled:
        yield
        return

    hidden_dir = exp_dir / ".hidden_agent_metadata"
    hidden_dir.mkdir(exist_ok=True)
    moved: list[tuple[Path, Path]] = []
    try:
        for rel in HIDDEN_AGENT_METADATA_PATHS:
            src = exp_dir / rel
            if not src.exists():
                continue
            dst = hidden_dir / rel
        ...
        if runs_dir.exists():
            for change_eval in runs_dir.glob("iteration_*/input/change_evaluation.json"):
                rel = change_eval.relative_to(exp_dir)
                dst = hidden_dir / rel
                dst.parent.mkdir(parents=True, exist_ok=True)
                if dst.exists():
                    dst.unlink()
                shutil.move(str(change_eval), str(dst))
                moved.append((change_eval, dst))
        ...
        try:
            if hidden_dir.exists() and not any(hidden_dir.rglob("*")):
                hidden_dir.rmdir()
        except OSError:
            pass
...
def apply_llm_provider_profile(config: dict) -> dict:
    """Apply optional provider profile from YAML to all agent LLM settings.
    ...
    active = provider_cfg.get("active")
    if not isinstance(profiles, dict) or not isinstance(active, str) or not active:
        return config

    profile = profiles.get(active)
    if not isinstance(profile, dict):
        print(f"[config] Warning: llm_provider.active={active!r} has no matching profile")
        return config

    llm = profile.get("llm")
    tool_call_mode = profile.get("tool_call_mode")
    ...
    if isinstance(llm, dict):
        ...
        evolve_llm = config.setdefault("evolve_agent", {}).setdefault("llm_config", {})
        adb_llm = config.setdefault("agent_debugger", {}).setdefault("llm", {})
        for k in ("api_key", "base_url", "model"):
            if llm.get(k):
                evolve_llm[k] = llm[k]
                adb_llm[k] = llm[k]

    if isinstance(tool_call_mode, str) and tool_call_mode:
        config.setdefault("code_agent_patch", {})["tool_call_mode"] = tool_call_mode
        config.setdefault("evolve_agent", {})["tool_call_mode"] = tool_call_mode
        config.setdefault("explore_agent_patch", {})["tool_call_mode"] = tool_call_mode

    if isinstance(api_type, str) and api_type:
        config.setdefault("code_agent_patch", {}).setdefault("llm_config", {})["api_type"] = api_type
        config.setdefault("evolve_agent", {}).setdefault("llm_config", {})["api_type"] = api_type
        config.setdefault("explore_agent_patch", {}).setdefault("llm_config", {})["api_type"] = api_type
        config.setdefault("agent_debugger", {}).setdefault("llm", {})["api_type"] = api_type

    if isinstance(reasoning, dict) and reasoning:
        config.setdefault("code_agent_patch", {}).setdefault("llm_config", {})["reasoning"] = copy.deepcopy(reasoning)
        config.setdefault("evolve_agent", {}).setdefault("llm_config", {})["reasoning"] = copy.deepcopy(reasoning)
        config.setdefault("explore_agent_patch", {}).setdefault("llm_config", {})["reasoning"] = copy.deepcopy(reasoning)
        config.setdefault("agent_debugger", {}).setdefault("llm", {})["reasoning"] = copy.deepcopy(reasoning)
    elif "reasoning" in profile and reasoning in (None, "__DELETE__", False):
        # Allow profiles (e.g. anthropic native) to explicitly disable reasoning
        # so existing per-agent defaults do not leak unsupported params.
        config.setdefault("code_agent_patch", {}).setdefault("llm_config", {}).pop("reasoning", None)
        config.setdefault("evolve_agent", {}).setdefault("llm_config", {}).pop("reasoning", None)
        config.setdefault("explore_agent_patch", {}).setdefault("llm_config", {}).pop("reasoning", None)
        config.setdefault("agent_debugger", {}).setdefault("llm", {}).pop("reasoning", None)

    print(f"[config] Applied llm_provider profile: {active}")
    return config
...
def load_config(config_path: str) -> dict:
    ...
    if "_base" not in raw:
        ...
        return apply_llm_provider_profile(resolved)
...
def apply_agent_yaml_patch(yaml_path: Path, patch: dict, label: str = "patch") -> None:
    ...
    def merge_patch(base: dict, overlay: dict) -> dict:
        result = copy.deepcopy(base)
        for key, value in overlay.items():
            if value == "__DELETE__":
                result.pop(key, None)
    ...
    patched = merge_patch(agent_config, patch)
...
def _build_harbor_cmd(config: dict, workspace_dir: Path, agent_config_filename: str,
    ...
    dataset_path_alias = resolve_dataset_path_alias(dataset)
    ...
    if dataset_path_alias:
        cmd.extend(["-p", str(dataset_path_alias)])
    elif task_path:
...
HIDDEN_RESULTS_ADB_QUERY = (
    "Summarize the provided agent execution trace(s). \n\n"
    "Return a concise, neutral summary covering:\n"
    "1. What the agent attempted to do.\n"
    "2. Important tool calls, artifacts, environment assumptions, and validation steps visible in the trace.\n"
    "3. Observable risks or fragile behavior, such as unsupported assumptions, missing validation, "
    "tool errors, working-directory drift, inefficient loops, or brittle setup.\n"
    "4. Reusable trace-grounded lessons, without using any result labels."
)
...
_adb_llm_cfg_lock = threading.Lock()
_adb_llm_cfg_applied: str | None = None
...
def _sync_adb_llm_config(llm_cfg: dict) -> bool:
    """Force adb CLI to use the provided LLM config (instead of stale persisted ~/.adb config)."""
    global _adb_llm_cfg_applied
    if not _ensure_adb_installed():
        return False

    adb_llm: dict[str, Any] = {}
    for key in ("model", "base_url", "api_key", "api_type"):
        val = llm_cfg.get(key)
        if val:
            adb_llm[key] = val
    max_tokens = llm_cfg.get("max_tokens")
    if isinstance(max_tokens, int) and max_tokens > 0:
        adb_llm["max_tokens"] = max_tokens
    for optional_key in ("reasoning", "thinking", "output_config"):
        val = llm_cfg.get(optional_key)
        if isinstance(val, dict) and val:
            adb_llm[optional_key] = val
    if "stream" in llm_cfg:
        adb_llm["stream"] = bool(llm_cfg["stream"])
    elif llm_cfg.get("api_type") == "anthropic_chat_completion":
        adb_llm["stream"] = True
    if not adb_llm:
        return False

    adb_cfg_json = json.dumps({"llm": adb_llm}, sort_keys=True)
    with _adb_llm_cfg_lock:
        if _adb_llm_cfg_applied == adb_cfg_json:
            return True
        adb = _adb_path or "adb"
        r = subprocess.run([adb, "config", adb_cfg_json], capture_output=True, text=True)
        if r.returncode == 0:
            _adb_llm_cfg_applied = adb_cfg_json
            return True
        err = (r.stderr or r.stdout or "").strip().replace("\n", " ")
        print(f"[adb] config failed: {err[:200]}", flush=True)
        return False
...
def _build_adb_jobs(
    ...
    hide_tests = hide_test_results_from_agents(config)
    ...
    for task_name, result in task_results.items():
        ...
        for d, reward_val in trial_dirs:
            ...
            # Collect verifier test output only when allowed for debugger/evolver prompts.
            test_stdout = d / "verifier" / "test-stdout.txt"
            if not hide_tests and test_stdout.exists():
                try:
        ...
        jobs.append(TaskAnalysisJob(
            ...
            mode="summary" if hide_tests else (
                "debug" if (n_fail > 0 or n_timeout > 0) else "summary"
            ),
...
def _invoke_adb_ask_once(cmd: list[str], env: dict[str, str], timeout: float) -> str:
    ...
    try:
        ...
        return f"[adb error] exit={result.returncode}: {detail[:500]}"
    except subprocess.TimeoutExpired:
...
def _run_single_adb_ask(job: TaskAnalysisJob, config: dict, k: int = 1,
    ...
    hide_results = hide_test_results_from_agents(config)
    ...
    if hide_results:
        query = HIDDEN_RESULTS_ADB_QUERY
    elif job.mode == "debug":
        if k == 1:
    ...
    if job.n_timeout > 0 and not hide_results:
        timeout_note = (
    ...
    llm_cfg.update(config.get("agent_debugger", {}).get("llm", {}) or {})
    _sync_adb_llm_config(llm_cfg)
    adb = _adb_path or "adb"
    ...
    if response.startswith("[adb"):
        ...
        if not hide_test_results_from_agents(config):
        ...
        if (job.is_timeout or job.n_timeout > 0) and not hide_results:
            for tp, rv in zip(job.trace_paths, job.trace_rewards):
                if rv < 0:
                    timing = _extract_trace_timing(tp)
                    if timing:
...
def _write_debugger_analyse(
    ...
    if hide_results:
        body_lines: list[str] = [
            f"Analyzed {len(results)} tasks. Result labels and reward values are hidden.\n",
        ]
        if results:
            body_lines.append("### Trace Analysis\n")
            for r in results:
    ...
    if timeout_results and not hide_results:
    ...
    if debug_results and not hide_results:
    ...
    if summary_results and not hide_results:
    ...
    for r in results:
        ...
        if hide_results:
            detail_lines = [
                f"# {r['task_name']}\n",
                f"Analyzed {n_total} trace(s). Result labels and reward values are hidden.\n",
            ]
        else:
        ...
        for idx, tp in enumerate(r["trace_paths"]):
            p = Path(tp)
            if hide_results:
                detail_lines.append(f"- **trace{idx+1:02d}**: `{tp}`")
            else:
        ...
        # Add verifier test output section for failing tasks only when results are visible.
        if not hide_results:
            has_verifier = False
...
def run_parallel_adb_ask(
    ...
    if hide_results:
        print(f"[adb] analyzing {len(jobs)} tasks (result labels hidden) "
    ...
    mode_suffix = "analysis" if hide_results else jobs[0].mode
    print(f"  [{status}] {jobs[0].task_name} ({mode_suffix}) [warmup]")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as pool:
            ...
            for future in concurrent.futures.as_completed(future_map):
                ...
                try:
                    ...
                    mode_suffix = "analysis" if hide_results else job.mode
                    print(f"  [{status}] {job.task_name} ({mode_suffix})")
                except Exception as e:
    ...
    analyse_dir, query_snippet = _write_debugger_analyse(
        results,
        iteration_dir,
        iteration,
        hide_results=hide_test_results_from_agents(config),
...
def update_history_before(exp_dir: Path, iteration: int, computed_stats: dict,
                          job_dir: Path, diff: dict | None = None,
                          hide_results: bool = False) -> None:
    ...
    if not history_path.exists():
        history_path.write_text("# Harness Evolution History\n\n", encoding="utf-8")
    ...
    if hide_results:
        lines.append(f"- **Evaluation completed** (each task x{k} rollout{'s' if k != 1 else ''})")
        lines.append("- Result counts, reward values, pass/fail labels, and verifier/test output are hidden from agents.")
    elif k > 1:
    ...
    if diff is not None and not hide_results:
    ...
    if hide_results:
        lines.append("\n### Task Details")
        lines.append("Hidden because `visibility.hide_test_results_from_agents` is enabled.")
    elif k > 1:
        lines.append(f"\n### Task Details")
        per_task_rollouts = computed_stats.get("per_task_rollouts", {})
...
def build_evolution_query(
    ...
    """
    k = stats.get("k", 1)
    hide_results = hide_test_results
    ...
    if hide_results:
        lines.append(f"- **Evaluation completed**: each task runs {k} time{'s' if k != 1 else ''}")
        lines.append("- Result counts, reward values, pass/fail labels, and verifier/test output are hidden.")
        lines.append("- Use only agent execution traces and any included ADB analysis grounded in those traces.")
    elif k > 1:
        trial_stats = stats.get("trial_stats", {})
        pass_at_k_data = stats.get("pass_at_k", {})
    ...
    if stats["exception_types"] and not hide_results:
    ...
    if timeout_exc_tasks and not hide_results:
    ...
    if pure_exc_tasks and not hide_results:
    ...
    # -- 5. Historical Trends --
    if scores_trend and len(scores_trend) >= 2 and not hide_results:
        if k > 1:
            lines.append(f"\n## 5. Historical Trends")
            for i in range(1, k + 1):
                pak_parts = []
    ...
    # -- 6. Best Ever --
    if best_ever and not hide_results:
        lines.append(f"\n## 6. Best Ever")
    ...
    # -- 7. Task Stability --
    if stability and not hide_results:
    ...
    if change_evaluation and not hide_results:
    ...
    if prev_variant_comparison and not hide_results:
    ...
    if hide_results:
        lines.append("**Important**: Result labels and reward values are hidden. Analyze only raw execution traces and ADB analysis that is grounded in trace evidence.")
    elif k > 1:
...
def run_evolve_agent(config: dict, exp_dir: Path, iteration: int,
    ...
    try:
        with hide_result_metadata_from_agents(
            exp_dir,
            hide_test_results_from_agents(config),
        ):
            raw_result = agent.run(
                message=query,
                context={
                    "date": datetime.now().strftime("%Y-%m-%d"),
                    "username": "rethinking-harness-evolution-ahe",
...
def run_multi_variant_adb(config: dict, variant_results: list[dict],
    ...
    for task_name in sorted(all_tasks):
        ...
        for vr in variant_results:
            ...
            for d in sorted(job_dir_v.iterdir()):
                ...
                label = f"variant_{vidx}" if hide_tests else (
                    "PASS" if rv >= 1.0 else ("TIMEOUT" if rv < 0 else "FAIL")
                )
    ...
    for vr in variant_results:
        ...
        if hide_tests:
            variant_header_parts.append(
                f"Variant {vidx} (strategy: {hint[:80]}): changes: {changes[:200]}"
            )
        else:
    ...
    def _run_one(job: TaskAnalysisJob) -> dict:
        ...
        extra_prefix = (
            ...
            + (
                "Compare across variants using only trace-observable behavior; do not infer or state pass/fail outcomes.\n"
                if hide_tests else
                "Compare across variants: which variant's approach was more effective and why?\n"
            )
        )
    ...
    for vr in variant_results:
        ...
        if hide_tests:
            body_lines.append(f"- **Variant {vidx}**{marker}: strategy: {hint[:100]}")
        else:
            vs = vr.get("stats", {})
            rate = vs.get("pass_rate", 0)
    ...
    if hide_tests:
        body_lines.append("### Trace Analysis\n")
        body_lines.append("Result labels and reward values are hidden.")
        for r in results:
            one_liner = _extract_one_liner(r["response"])
            body_lines.append(f"- **{r['task_name']}**: {one_liner}")
        body_lines.append("")

    if timeout_results and not hide_tests:
    ...
    if debug_results and not hide_tests:
        body_lines.append("### Debug (has failures)\n")
        for r in debug_results:
            one_liner = _extract_one_liner(r["response"])
    ...
    if summary_results and not hide_tests:
        names = ", ".join(r["task_name"] for r in summary_results)
        body_lines.append(f"### Summary (all pass): {len(summary_results)} tasks\n")
        body_lines.append(f"{names}\n")
        body_lines.append("(All rollouts passed — no per-task analysis included. Read `detail/{{task}}.md` if needed.)\n")
    ...
    for r in results:
        ...
        for idx, tp in enumerate(r.get("trace_paths", [])):
            if hide_tests:
                trace_section += f"- trace{idx+1:02d}: `{tp}`\n"
            else:
...
def run_best_of_n_evolution(
    ...
    for i in range(n):
        ...
        query = build_evolution_query(
            ...
            hide_test_results=hide_test_results_from_agents(config),
        )
...
def run_single_experiment(config: dict, config_path: str, experiment_name: str | None = None,
    ...
    print(f"\n{'='*60}")
    print(f"Harness Evolution Automated Evolution System")
    ...
    try:
        for iteration in range(start_iteration, max_iterations + 1):
            ...
            eval_skipped_this_iteration = False
            ...
            if bon_enabled and _bon_prev_winner is not None:
                ...
                try:
                    if skip_eval:
                        ...
                        eval_skipped_this_iteration = True
                        skip_eval = False
            ...
            if adb_config.get("enabled") and not reusing_bon_winner:
                ...
                detail_dir = iteration_dir / "input" / "analysis" / "detail"
                can_reuse_overview = (
                    existing_overview.exists()
                    and (
                        not hide_test_results_from_agents(config)
                        or eval_skipped_this_iteration
                    )
                )
                if can_reuse_overview and eval_skipped_this_iteration:
                    expected_jobs = _build_adb_jobs(
                        stats["task_results"],
                        job_dir,
                        adb_config,
                    ...
                    if expected_count == 0:
                        print(
                            "[adb] overview.md exists but no tasks to analyze; "
                            "re-running agent debugger",
                            flush=True,
                        )
                        can_reuse_overview = False
                    else:
                        missing = [
                            j.task_name for j in expected_jobs
                            if not (detail_dir / f"{j.task_name}.md").exists()
                        ]
                        if missing:
                            have = expected_count - len(missing)
                            print(
                                f"[adb] overview.md exists but detail files incomplete "
                                f"({have}/{expected_count}); re-running agent debugger",
                                flush=True,
                            )
                            can_reuse_overview = False
                if can_reuse_overview:
                    ...
                    reuse_label = (
                        "skip-eval resume"
                        if eval_skipped_this_iteration and hide_test_results_from_agents(config)
                        else "existing analysis"
                    )
                    print(
                        f"[adb] Reusing {reuse_label} from "
                        f"{existing_overview.relative_to(exp_dir)}"
                    )
                else:
                    print(
                    ...
                    effective_adb_config = copy.deepcopy(adb_config)
                    effective_adb_config["visibility"] = config.get("visibility", {})
                    adb_overview = run_parallel_adb_ask(
                        config=effective_adb_config,
                        job_dir=job_dir,
                        task_results=stats["task_results"],
                        iteration_dir=iteration_dir,
                        iteration=iteration,
...
def run_batch(config_paths: list[str]) -> None:
    ...
    for config_path in config_paths:
        ...
        cmd = f"cd {PROJECT_DIR} && python evolve_ahe.py --config {Path(config_path).resolve()}"
...
def main():
    parser = argparse.ArgumentParser(description="Harness Evolution Automated Evolution Evaluation System")
    parser.add_argument(
        "--config", default=None,
